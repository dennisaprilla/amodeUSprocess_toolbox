...
% [allpeaks, processed_USsignal] = peaks_USsignal_windowed(USData, data_spec, us_spec, windowRange, windowRange_i);
% 
% % clear unneccesary variable
...
axes1 = subplot(4,2,[1 3 5 7], 'Parent', figure1); % for 3d trajectory
axes2 = subplot(4,2,4, 'Parent', figure1);
axes3 = subplot(4,2,6, 'Parent', figure1);
% axes4 = subplot(3,2,6, 'Parent', figure1);       % for bar plot
...
for current_frame = 1:data_spec.n_frames
    ...
    % Reconstructed A-mode Trajectory -------------------------------------
    % plot the amode
    ...
    % A-mode signal -------------------------------------------------------
    % plot a-mode    
    delete(findobj('Tag', 'plot_raw1'));
    delete(findobj('Tag', 'plot_peak1'));
    plot(axes2, x_axis_values, USData(29, :, current_frame), '-', 'Color', 'g', 'Tag', 'plot_raw1');
    xlabel(axes2, 'Distance (mm)');
    ylabel(axes2, 'Signal Amplitude');
    ylim(axes2, [-5000, 5000]);
    hold(axes2, 'on');
    xline(axes2, allpeaks.locations(29,current_frame), '-', sprintf('peak'), 'Color', 'b',  'Tag', 'plot_peak1');
    title(axes2, sprintf("A-Mode Probe #%d", 29));
    
    
    delete(findobj('Tag', 'plot_raw2'));
    delete(findobj('Tag', 'plot_raw2'));
    plot(axes3, x_axis_values, USData(30, :, current_frame), '-', 'Color', 'g', 'Tag', 'plot_raw2');
    xlabel(axes3, 'Distance (mm)');
    ylabel(axes3, 'Signal Amplitude');
    ylim(axes3, [-5000, 5000]);
    hold(axes2, 'on');
    xline(axes3, allpeaks.locations(30,current_frame), '-', sprintf('peak'), 'Color', 'b',  'Tag', 'plot_peak2');
    title(axes3, sprintf("A-Mode Probe #%d", 30));
    
%     % Bar plot for depth --------------------------------------------------
%     delete(findobj('Tag', 'bar_depth'));
%     bar(axes4, (start_ust:30), allpeaks.locations(start_ust:end, current_frame), 'Tag', 'bar_depth', 'FaceColor', 'blue');
%     grid(axes4, 'on');
%     xlabel(axes4, 'Ultrasound Transducer #');
%     ylabel(axes4, 'Depth (mm)');
%     ylim(axes4, [0 10]);
%     title(axes4, 'Depth measurements');
...
clear current_frame current_ust offset_ustext
